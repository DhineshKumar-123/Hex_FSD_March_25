 ...
 */
/**
 * Now create the edit logic for put mapping API in frontend for the backend
 */
function StudentList() {
    ...
    const getAllStudents = async () => {
        try {
            const response
                = await axios.get(`http://localhost:8080/api/student/all?page=${page}&size=${size}`)
    ...
    */
    const deleteStudents = (sid) => {
        ...
        try {
            const resp = axios.delete(`http://localhost:8080/api/student/delete/${sid}`);
            let temp = [...students];
        ...
        //getAllStudents() -- dont call API unless its very necessary so that it wont delete immediately . 
        const updateStudent = async ($e, sid) => {
            $e.preventDefault();
            try {
                const response = await axios.put('http://localhost:8080/api/student/update/' + sid, {
                    'name': name,
                    'age': age
                });
                            ...
                            students.sort((a, b) => a.id - b.id).map((s, index) => (
                                <tr key={index}>
                                    ...
                                    <td>
                                        ...
                                        &nbsp;&nbsp;&nbsp;
                                        <button className="btn btn-info btn-sm" data-bs-toggle="modal" data-bs-target={`#update-${s.id}`}>Update</button>
                                    </td>

                                    {/** Modal Box*/}
                                    <div className="modal fade" id={`update-${s.id}`} tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
